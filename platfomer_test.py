...
class GameView(arcade.View):
    def __init__(self):
        ...
        #  Buat score dan score text
        self.score = 0
        # self.score_text = arcade.Text(f"Score: {self.score}", 10, self.height - 30, arcade.color.WHITE, 18)
    ...
    def load_level(self,level):
        ...
        # Get coins
        # coin_objects = self.tile_map.object_lists.get("coins")
        # print(f"List available coins {coin_objects}")
        # coin_layer = self.tile_map.sprite_lists["coins"]
        # print(f"List available coin layer {coin_layer}")
        print("Sprite layers:", self.tile_map.sprite_lists.keys())
        print("Object layers:", self.tile_map.object_lists.keys())
        ...
        if spawn_points:
            for obj in spawn_points:
                ...
                if obj.name == "player_spawn":
                    # Koordinat x dan y dari obj.shape
                    x = obj.shape[0]
                    y = obj.shape[1]

                    # Flip koordinat y agar sesuai dengan Arcade
                    y = map_height - y
                    self.player_sprite.center_x = x
                    self.player_sprite.center_y = y
                    # print(f"Set player spawn at x:{x}, y:{y}")
                    break
        # Load the items -> card
        # items_layer = self.tile_map.object_lists.get("Items", [])
        ...
        with self.gui_camera.activate():
            ...
            if self.game_over:
                arcade.draw_text("GAME OVER!\nPress ESC to Restart",
                                 ...
                                 arcade.color.RED, 36, anchor_x="center", anchor_y="center")


        arcade.draw_text(f"Score: {self.score}", 10, self.window.height - 30, arcade.color.WHITE, 18)
    ...
    def on_update(self, delta_time):
        ...
        if not self.game_over:
            if self.player_sprite.center_y < -50:
                self.game_over = True
                print("Game Over: Player jatuh!")

            self.physic_engine.update()
            # Posisi camera ikut player
            ...
            for teleport in hit_list:
                ...
                if target_name and target_name in self.teleport_targets:
                    ...
                    y = self.map_height - y  # convert y ke koordinat arcade
                    self.player_sprite.center_x = x
                    self.player_sprite.center_y = y
                    print(f"Teleported to {target_name} at ({x},{y})")
                    break
        ...
        for coin in coins_hit:
            coin.remove_from_sprite_lists()  # Hilangkan coin dari layar
            self.score += 1
            print(f"Kumpul coin! Skor: {self.score}")
